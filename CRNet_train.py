...
from models.CRNet_model import BBSNet
...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            s0,s1,s2,s3,s4,s5,s0_sig,s1_sig,s2_sig,s3_sig,s4_sig,s5_sig = model(images,depths)
            ...
            loss = 0.1*loss0 + 0.3*loss1 + 0.5*loss2+ 0.7*loss3 + 0.9*loss4 + 1*loss5
            loss.backward()
            ...
            if i % 100 == 0 or i == total_step or i==1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}],  Loss0: {:.4f} Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:0.4f} Loss4: {:0.4f} Loss5: {:0.4f} Total_Loss: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss0.data, loss1.data, loss2.data,loss3.data,loss4.data,loss5.data,loss.data))
...
def test(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            image = image.cuda()
            depth = depth.cuda()
            _,_,_,_,_,res,_,_,_,_,_,_ = model(image,depth)
