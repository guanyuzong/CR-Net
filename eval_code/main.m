...
for i = 1:numel(algorithms)
    ...
    for j = 1:numel(datasets)     
        dataset      = datasets{j};
        predpath     = ['D:\wenzhang\code\CR-Net\test_maps\' alg '/' dataset '/'];
        maskpath     = ['D:\wenzhang\code\CR-Net\CR_dataset\RGBD_for_test\' dataset '/GT/'];
        if ~exist(predpath, 'dir'), continue; end
        if strcmp(alg, 'PoolNet') && strcmp(dataset, 'HKU-IS'), names = textread(['../data/' dataset '/test_poolnet.txt'], '%s'); 
        else names = textread(['D:\wenzhang\code\CR-Net\CR_dataset\RGBD_for_test/' dataset '/test.txt'], '%s'); end
        ...
        for k = 1:numel(names)  
            ...
            results{k, 1} = name;
            file_num(k)   = true;
            if strcmp(alg, 'PoolNet-RGB'), fgpath = [predpath name '_sal_fuse.png'];
        ...
        sm       = sm  / sum(file_num); 
        em       = em  / sum(file_num);
        fprintf('%s: maxFmeasure: %6.4f, Fmeasure: %6.4f, wFmeasure: %6.4f, Emeasure:%6.4f, MAE: %6.4f, Smeasure: %6.4f\n', dataset, maxF,fm, wfm,em, mae,sm);
        %fprintf('%s: maxFmeasure: %6.4f, wFmeasure: %6.4f, MAE: %6.4f\n', dataset, maxF, wfm, mae);
        %fprintf('%s: maxFmeasure: %6.4f, MAE: %6.4f, sMeasure: %6.4f\n', dataset, maxF, mae, sm);

        save_path = ['./rgbd_result' filesep alg filesep dataset filesep];
